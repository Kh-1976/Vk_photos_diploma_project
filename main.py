...
class YaUploader:
  ...
  def new_folder(self):
    ...
    if response.status_code == 201:
      ...
      print('Папка New_Folder уже существует')

  def upload_photo(self):
    upload_url = 'https://cloud-api.yandex.net/v1/disk/resources/upload'
    link = vk.search_photo(vk.photos())
    for key, val in link.items():
        params = {'path': f'New_Folder/{key}', 'url': val}
    ...
    def search_photo(self, lst_photos):
        ...
        for dict_i in lst_photos:
            if str(dict_i['likes']['count']) not in no_repeat:
                ...
                dict_links[str(dict_i['likes']['count']) + '_' + str(dict_i['date'])+'.jpg'] = dict_i[f'photo_{str(self.max_resolution(dict_i))}']
